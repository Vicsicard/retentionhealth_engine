...
function json(data: unknown, status = 200): Response {
  return new Response(JSON.stringify(data, null, 2), {
    ...
    headers: {
      ...
      "access-control-allow-headers": "Content-Type, x-admin-key, x-clinic-id",
    },
  });
}
...
async function routeSubmitMetrics(request: Request, env: Env): Promise<Response> {
  ...
  if (!clinicId) {
    return badRequest('x-clinic-id header required');
  }
...
async function routeCreateClinic(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare(
    'INSERT INTO clinics (id, name, pilot_status) VALUES (?, ?, ?)'
  ).bind(clinicId, body.name, 'active').run();
