...
// ============================================================================
// POST /api/pilot-request - Handle pilot application form submissions
// ============================================================================
async function routePilotRequest(request: Request, env: Env): Promise<Response> {
  try {
    const body = await request.json().catch(() => null) as any;
    const { clinicName, contactName, email, phone, activePatients } = body;

    if (!clinicName || !contactName || !email || !phone || !activePatients) {
    ...
    // Notification email to contact@retentionhealth.com
    const notificationEmail = {
      personalizations: [{
        to: [{ email: NOTIFICATION_EMAIL }],
        subject: `New 8-Week Pilot Application — ${clinicName}`,
      }],
      from: { email: 'noreply@retentionhealth.com', name: 'RetentionHealth' },
      content: [{
        type: 'text/html',
        value: `
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (path === '/api/pilot-request' && request.method === 'POST') {
        return await routePilotRequest(request, env);
      }
